...
%         angleError = atan2d( vecnorm(cross(v_1,v_2,2),2,2) , dot(v_1,v_2,2));
        ...
        u = startSearchLoc-targetLoc ; 
        v = startSearchLoc-dropLoc ;  
        angleError = acosd(dot(u,v) / (norm(u)*norm(v)));
        signed_angleError = atan2d(u(1)*v(2)-u(2)*v(1),u(1)*v(1)+u(2)*v(2));
        ...
        trialInfo_furtherProcess(trialNo).signed_angleError = signed_angleError;
...
%             pathLen = sum(test(:,1),1);
            ...
            % non-moving ratio
            trialInfo_furtherProcess(trialNo).nonMovingRatio = sum(linearSpeed==0)/length(linearSpeed);
